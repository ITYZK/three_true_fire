...
class orders(models.Model):
    ...
    otime = models.DateTimeField() # 下单时间
...
class order_items(models.Model):
    oi_id = models.AutoField(auto_created=True,primary_key=True) 
...
class user_info(models.Model):
    nickname = models.ForeignKey(user_acount, to_field="nickname", on_delete=models.CASCADE) # 用户名（账号昵称）
    realname = models.CharField(max_length=100, unique=True) # 真名
    phone = models.CharField(max_length=11) # 默认电话
    target_addr = models.CharField(max_length=255) # 默认地址
