...
from .models import user_acount, foods,orders,order_items
...
def order(req):
    if req.method == "GET":
        ...
        order_name = req.POST.get("order_name")   #获取订单的单品菜名列表
        order_count = req.POST.get("order_count") #获取订单的单品数量列表
        order_price = req.POST.get("order_price") #获取订单的单品总价格列表
        sum_pic = req.POST.get("sum_price")       #获取订单的总价格
